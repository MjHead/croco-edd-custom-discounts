...
class EDD_Discount {
	...
	public function __construct( $_id_or_code_or_name = false, $by_code = false, $by_name = false ) {
		...
		$this->custom_discount = apply_filters(
			'edd_custom_discount',
			false,
			$_id_or_code_or_name,
			$by_code,
		...
		if ( $this->custom_discount ) {
			...
			$this->ID                = $this->custom_discount->get_ID();
			$this->name              = $this->custom_discount->get_name();
			$this->code              = $this->custom_discount->get_code();
			$this->status            = $this->custom_discount->get_status();
			$this->post_status       = $this->custom_discount->get_status();
			$this->type              = $this->custom_discount->get_type();
			$this->amount            = $this->custom_discount->get_amount();
			$this->product_reqs      = $this->custom_discount->get_product_reqs();
			$this->excluded_products = $this->custom_discount->get_excluded_products();
			$this->start             = $this->custom_discount->get_start();
			$this->expiration        = $this->custom_discount->get_expiration();
			$this->uses              = $this->custom_discount->get_uses();
			$this->max_uses          = $this->custom_discount->get_max_uses();
			$this->min_price         = $this->custom_discount->get_min_price();
			$this->is_single_use     = $this->custom_discount->get_is_single_use();
			$this->is_not_global     = $this->custom_discount->get_is_not_global();
			$this->product_condition = $this->custom_discount->get_product_condition();
	...
	public function __get( $key ) {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'get_' . $key ) ) {
			return call_user_func( array( $this->custom_discount, 'get_' . $key ) );
		} elseif ( $this->custom_discount ) {
			return false;
		}
	...
	public function __set( $key, $value ) {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'set_' . $key ) ) {
			return call_user_func( array( $this->custom_discount, 'set_' . $key ) );
		}
	...
	private function setup_discount( $discount = null ) {
		 ...
		 */
		$discount_object = apply_filters( 'edd_pre_setup_discount_object', false, $this, $discount );
	...
	public function get_meta( $key = '', $single = true ) {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'get_meta' ) ) {
			return $this->custom_discount->get_meta( $key );
		}
	...
	public function update_meta( $key = '', $value = '', $prev_value = '' ) {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'update_meta' ) ) {
			$this->custom_discount->update_meta( $key, $value );
			return;
		}
	...
	public function update( $args ) {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'update' ) ) {
			$this->custom_discount->update( $args );
			return;
		}
	...
	public function update_status( $new_status = 'active' ) {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'update_status' ) ) {
			$this->custom_discount->update_status( $new_status );
			return;
		}
	...
	public function increase_usage() {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'increase_usage' ) ) {
			$this->custom_discount->increase_usage();
			return;
		}
	...
	public function decrease_usage() {
		...
		if ( $this->custom_discount && method_exists( $this->custom_discount, 'decrease_usage' ) ) {
			$this->custom_discount->decrease_usage();
			return;
		}
